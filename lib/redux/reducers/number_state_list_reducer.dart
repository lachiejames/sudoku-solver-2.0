...
final Reducer<List<NumberState>> numberStateListReducer = combineReducers<List<NumberState>>([
  ...
  TypedReducer<List<NumberState>, RemoveValueFromTileAction>(_makeNumberInactive2),
  TypedReducer<List<NumberState>, NumberPressedAction>(_makeNumberInactive3),
]);
...
List<NumberState> _makeNumberInactive2(List<NumberState> numberStateList, RemoveValueFromTileAction action) {
  List<NumberState> newNumberStateList = List<NumberState>();
  numberStateList.forEach((numberState) {
    newNumberStateList.add(numberState.copyWith(isActive: false));
  });
...
List<NumberState> _makeNumberInactive3(List<NumberState> numberStateList, NumberPressedAction action) {
  List<NumberState> newNumberStateList = List<NumberState>();
  numberStateList.forEach((numberState) {
    newNumberStateList.add(numberState.copyWith(isActive: false));
  });
